...
function hero_getHeroDetail($caveID, $meineHoehlen){
	...
	$playerID = $params->SESSION->user['playerID'];
	...
	$monster_msng = "oder schicke Helden zu einem Monsterduell ...";
	...
	if (isset($params->POST->monster)) {
		...
		if (!$xCoord || !$yCoord)
			$monster_msng = "Sie sollten die Koordinaten der Zielsiedlung setzen, denn erst dann ".
								"geht Held zum Monsterkampf ...";
		else if ($xCoord <= 0 || $yCoord <= 0)
			$monster_msng = "Die Koordinaten der Siedlungen in in diesem Spiel k&ouml;nnen nicht negativ ".
			...
			if ($monster_cave == -2)
				...
				$monster_msng = "Es existiert keine Siedlung mit Koordinaten ".$xCoord."/".$yCoord.
	...
	if (isset($params->POST->turnier)) {
		...
		if ($turnierID < 0)
			$turnier_msng = "W&auml;hle zuerst <b>ein Turnier</b> und gebe danach noch das Gebot ein";
		else if ($meingebot < 0)
...
/* **** CREATE HERO PAGE ***** *********************************************/
	...
	// get the template
	$template = tmpl_open('./templates/' . $config->template_paths[$params->SESSION->user['template']] . '/hero_detail.ihtml');
