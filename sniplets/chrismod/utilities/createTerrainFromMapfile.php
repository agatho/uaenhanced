...
while (($c = fgetc($fp)) != FALSE){
  ...
  switch ($c){
    case '.':
            array_push($terrain, TERRAIN_PLAINES);
            break;
    case 'T':
            array_push($terrain, TERRAIN_FOREST);
            break;
    case 'M':
            array_push($terrain, TERRAIN_MOUNTAIN);
            break;
    case '~':
            array_push($terrain, TERRAIN_SWAMP);
            break;
    default:
...
// ADDED by chris--- cause its better, 3.8.2004
//  $query = "UPDATE Cave SET starting_position = 1 ORDER BY RAND() LIMIT ".$limit;
  $query = "UPDATE Cave SET starting_position = 1 WHERE starting_position = 0 AND playerID = 0 ORDER BY RAND() LIMIT ".$limit;
  if (!$db->query($query)){
