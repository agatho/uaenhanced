...
<xsl:template match="Resource">
  $tmp = new Resource(<xsl:value-of select="count(preceding-sibling::*)"/>, '<xsl:value-of select="Name"/>', '<xsl:apply-templates select="@id"/>', '<xsl:apply-templates select="MaxStorage"/>',
...
class Building {
  ...
  function Building($buildingID, $name, $description, $dbFieldName, $position, $maxLevel, $productionTimeFunction, $ratingValue){
...
<xsl:template match="Building">
  ...
  $tmp-&gt;externalProductionCost = array(<xsl:apply-templates select="Cost[name(id(@id))='DefenseSystem']"/>);
  </xsl:if>
