...
function questionnaire_getQuestionnaire($caveID, &$meineHoehlen){
  ...
  $template = tmpl_open($params->SESSION->player->getTemplatePath() . 'questionnaire.ihtml');
  ...
  // show my credits
  if ($account = questionnaire_getCredits($params->SESSION->player->questionCredits))
...
function questionnaire_getQuestions(){
  ...
  $query = "SELECT * FROM Questionnaire_answers ".
           "WHERE playerID = {$params->SESSION->player->playerID} ".
...
function questionnaire_giveAnswers(){
  ...
  if (!($result = $db->query($query))) return _('Fehler') . ": " . mysql_error();
  if ($result->isEmpty()) return _('Keine derartigen Fragen!');
  ...
  foreach ($answers AS $questionID => $choiceID){
    $query = "INSERT INTO Questionnaire_answers ".
             ...
             "VALUES ('{$params->SESSION->player->playerID}', ".
             "'$questionID', '$choiceID')";
    if (!($result = $db->query($query))) return _('Fehler') . ": " . mysql_error();
...
function questionnaire_addCredits($credits){
  ...
  $query = "UPDATE Player SET questionCredits = questionCredits + $credits WHERE playerID = " .
           $params->SESSION->player->playerID ." AND questionCredits + $credits >= 0";
  if (!$db->query($query)) die("questionnaire_addCredits: " . mysql_error());

  if ($db->affected_rows() != 1) return false;

  $params->SESSION->player->questionCredits += $credits;
  $_SESSION['player']->questionCredits = $params->SESSION->player->questionCredits;
  return true;
}
...
function questionnaire_presents($caveID, &$meineHoehlen){
  ...
  $template = tmpl_open($params->SESSION->player->getTemplatePath() . 'questionnaire_presents.ihtml');
...
function questionnaire_getPresent($caveID, &$meineHoehlen, $presentID){
  ...
  // genügend Schnecken?
  $myaccount = questionnaire_getCredits($params->SESSION->player->questionCredits);
